...
long int extractExifInfo ( char *fileName,
    ...
    if ( ( exifMarkerPos 
           = findMarkerInFile ( fp, exifMarker, EXIF_MARKER_LENGTH, 
    ...
    if ( ( rc = addIfdToQueue ( &ifdQueue, &ifdQueueItemCount,
                                ...
                                + EXIF_HEADER_LENGTH, IFD_ID_IFD) ) < 0 )
...
static long int getIfdLink ( FILE *fp,
    ...
    if ( fseek ( fp, ifdPos
                     + IFD_HEADER_LENGTH
                     + EXIF_TAG_LENGTH * ifdTagCount, SEEK_SET )  != 0 )
        return EXIF_ERR_FILE_READ;
...
static long int allocateExifTable ( struct exifItem **exifTable,
    ...
    if ( exifTableItemCount == 0 ) {
        ...
        if( ( *exifTable =
              ( struct exifItem * ) malloc ( sizeof ( struct exifItem ) ) )
            == NULL )
            return EXIF_ERR_MALLOC;
        ...
        if ( ( *exifTable =
              ( struct exifItem * ) realloc ( *exifTable,
                                              sizeof ( struct exifItem ) 
                                              * ( exifTableItemCount + 1 ) ) )
            == NULL )
            return EXIF_ERR_MALLOC;
...
static long int addIfdToExifTable ( struct exifItem **exifTable,
    ...
    if ( ( ifdLink =
           getIfdLink ( fp, ifdPos, ifdTagCount, exifFormat  ) )
           < 0 )
        return ifdLink;
